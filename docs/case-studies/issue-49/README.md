...
- **A1** Attach user's _current active_ avatar to the invitation post (upstream
...
- **B1** "Reset to default" inside _Invitation messages_ must persist the reset
...
- **D4** Provide a UI **Clear log** button alongside the existing _Copy log_
  button.
...
- **G1** Use `account.getAppPermissions` _before_ failing on code 3 and include
...
**Fix direction:** flip the contract so the form serialises the _current
display value_ unconditionally and bypasses the diff check on explicit
...
- ✅ Call `account.getAppPermissions` once at startup (and once when we first
  ...
  _supposed_ to come with `scope = 1073737727` (all bits set), but if a
...
| A3       | Per cycle, `wall.get({ owner_id, count: 10 })`. If any cached post is still in the top 10, skip post + skip delete. Else delete _and_ post fresh.                                                                                                                           | `send-invitation-posts.js`                                                                                                                                                            |
...
Kate Mobile). VK enforces the new restriction _above_ the scope-check layer,
