...
// not grant env or write access, but `os.tmpdir()` reads TMPDIR/TEMP env vars
// and the test writes log files. Skip under Deno; Node and Bun still exercise
// the suite fully.
...
describe(
  'openSessionLog: writes log lines to a session file',
  { skip: isDenoRuntime },
  () => {
...
describe(
  'openSessionLog: redacts secrets in persisted logs',
  { skip: isDenoRuntime },
  () => {
