...
// The forked-child regression test below spawns `process.execPath`, which
// requires env access. The Deno CI job runs node:test under Deno without
// `--allow-env`, so spawn() throws `NotCapable`. Skip the spawn-based case on
// Deno; the in-process scheduling case still runs on every runtime.
...
describe('startBot scheduling', () => {
  ...
  it(
    'does not exit prematurely when run as a forked child (regression for issue #32)',
    { skip: isDenoRuntime },
    async () => {
